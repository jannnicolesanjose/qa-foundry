...
- **Lunch break, in hours**: type a decimal value up to 1 hour — `1` for a full hour, `0.5` for 30 minutes, `0.25` for 15 minutes, and so on. This is the only thing deducted from worked hours: Hours = (Time Out − Time In) − Lunch Break. Go over 1 hour and it's clamped automatically with a heads-up toast. This math has been verified line by line against real entries, including cases where Time In is before 8:00 AM.
...
- A small "Filter" label sits above the From/To date-range controls so it's clear that section filters the sheet rather than being part of the data itself.
- **Leave tracking, in days, with a running balance**: click "Add leave" for a row with a Leave Type dropdown (Paid Leave or Sick Leave) and a directly-editable Days field (defaults to 1). Time In/Out and Lunch Break aren't applicable to leave rows and show as disabled.
  - A **Leave Balance** panel sits above the sheet with Paid Leave and Sick Leave tiles — each shows days used and days remaining, calculated against an editable allowance (defaults to **14 days paid / 10 days sick**, matching a standard contract; change the number directly in the tile if yours differs). This balance is based on every leave entry you've logged, independent of whatever date filter is active.
  - A reminder note field underneath is pre-filled with a note that the balance only reflects leave logged in this sheet — edit it freely to track anything else worth remembering (e.g. leave already approved elsewhere but not yet entered here).
- Filter by date range (or use "This week") and see live totals: hours (including leave, counted as an 8-hour day), Lunch Break Total, entries, days logged, and today's hours.
- **Export** the currently filtered range as — every centered column stays centered both horizontally and vertically in all three, not just on screen (verified against the actual generated files, not just eyeballed), and leave rows show up clearly labeled with their day count (e.g. "LEAVE — Paid Leave" with "2.00 days" in the Hours column):
  - **Excel (.xlsx)** — a proper spreadsheet, via ExcelJS, with the employee/pay-period header and totals included.
  - **PDF** — a formatted table with a header, date range, and totals, via jsPDF, titled "Personal Timesheet."
